  ...
  return (
    <div>
        ...
        {items.map((item, index) => {
          ...
          return (
            ...
            >
              ...
              {item.type === 'work' && <span style={s.workBadge}>Work</span>}
      ...
      <button
        onClick={handleSave}
        disabled={saving}
        style={{ ...s.saveOrderBtn, color: saved ? '#22c55e' : 'var(--theme-text)' }}
      >
    ...
    <div style={s.page}>
      ...
      <section style={s.section}>
        <div style={s.sectionHeader}>
          <p style={s.sectionTitle}>Contact Page</p>
        </div>
        <div
          style={s.singletonCard}
          onClick={() => router.push('/admin/globals/contact')}
        ...
        >
          <div style={s.singletonLeft}>
            <p style={s.singletonTitle}>Contact</p>
            <span style={s.singletonMeta}>Links, email og recognitions</span>
          </div>
      ...
      <section style={s.section}>
        <div style={s.sectionHeader}>
          <p style={s.sectionTitle}>Awards</p>
        </div>
        <div
          style={s.singletonCard}
          onClick={() => router.push('/admin/globals/awards')}
          onMouseEnter={e => ((e.currentTarget as HTMLDivElement).style.background = 'var(--theme-elevation-100, rgba(255,255,255,0.06))')}
          onMouseLeave={e => ((e.currentTarget as HTMLDivElement).style.background = 'var(--theme-elevation-50, rgba(255,255,255,0.03))')}
        >
          <div style={s.singletonLeft}>
            <p style={s.singletonTitle}>Awards</p>
            <span style={s.singletonMeta}>Klistermærker til projekter</span>
          </div>
          <span style={s.editLink}>Rediger →</span>
        </div>
      </section>
      ...
      <section style={s.section}>
        ...
        {loading ? (
          ...
          <ProfileOrder profiles={profiles} work={work ?? {}} onSave={saveOrder} />
        )}
      </section>
...
const s: Record<string, React.CSSProperties> = {
  page: { padding: '40px 48px', fontFamily: 'var(--font-body)', color: 'var(--theme-text)', maxWidth: 960 },
  backButton: {
    fontSize: 13, fontWeight: 500, color: 'var(--theme-text)',
    background: 'var(--theme-elevation-50)', border: '1px solid var(--theme-elevation-150)',
    borderRadius: 4, padding: '6px 14px', marginBottom: '24px',
    cursor: 'pointer', opacity: 0.45, transition: 'opacity 0.15s', whiteSpace: 'nowrap',
  },
  heading: { fontSize: 13, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--theme-text-secondary, #888)', marginBottom: 24, marginTop: 0 },
  section: { marginBottom: 48 },
  sectionHeader: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 },
  sectionTitle: { fontSize: 13, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--theme-text-secondary, #888)', margin: 0 },
  newButton: { fontSize: 12, fontWeight: 500, color: 'var(--theme-text)', background: 'none', border: '1px solid var(--theme-border-color, rgba(255,255,255,0.15))', borderRadius: 4, padding: '4px 12px', cursor: 'pointer', letterSpacing: '0.04em' },
  card: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', border: '1px solid var(--theme-border-color, rgba(255,255,255,0.1))', borderRadius: 6, marginBottom: 8, background: 'var(--theme-elevation-50, rgba(255,255,255,0.03))', cursor: 'pointer', transition: 'background 0.15s' },
  cardTitle: { fontSize: 14, fontWeight: 500, margin: 0, color: 'var(--theme-text)' },
  cardMeta: { fontSize: 12, color: 'var(--theme-text-secondary, #888)', margin: 0, marginTop: 2 },
  editLink: { fontSize: 12, color: 'var(--theme-text-secondary, #888)', letterSpacing: '0.04em' },
  singletonCard: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 18px', border: '1px solid var(--theme-border-color, rgba(255,255,255,0.1))', borderRadius: 6, background: 'var(--theme-elevation-50, rgba(255,255,255,0.03))', cursor: 'pointer' },
  singletonLeft: { display: 'flex', flexDirection: 'column', gap: 4 },
  singletonTitle: { fontSize: 14, fontWeight: 500, margin: 0, color: 'var(--theme-text)' },
  singletonMeta: { fontSize: 12, color: 'var(--theme-text-secondary, #888)' },
  emptyState: { fontSize: 13, color: 'var(--theme-text-secondary, #888)', padding: '14px 18px', border: '1px dashed var(--theme-border-color, rgba(255,255,255,0.1))', borderRadius: 6, textAlign: 'center' },
  divider: { border: 'none', borderTop: '1px solid var(--theme-border-color, rgba(255,255,255,0.08))', margin: '0 0 48px 0' },
  orderList: { display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 12 },
  orderItem: { display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', border: '1px solid var(--theme-border-color, rgba(255,255,255,0.1))', borderRadius: 6, background: 'var(--theme-elevation-50, rgba(255,255,255,0.03))', cursor: 'grab', userSelect: 'none' },
  dragHandle: { color: 'var(--theme-text-secondary, #888)', fontSize: 14, flexShrink: 0 },
  orderItemName: { fontSize: 13, fontWeight: 500, color: 'var(--theme-text)', flex: 1 },
  workBadge: { fontSize: 10, fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--theme-text-secondary, #888)', border: '1px solid var(--theme-border-color, rgba(255,255,255,0.15))', borderRadius: 3, padding: '2px 6px' },
  orderIndex: { fontSize: 11, color: 'var(--theme-text-secondary, #888)' },
  saveOrderBtn: { fontSize: 12, fontWeight: 500, background: 'none', border: '1px solid var(--theme-border-color, rgba(255,255,255,0.15))', borderRadius: 4, padding: '5px 14px', cursor: 'pointer', transition: 'color 0.15s' },
}